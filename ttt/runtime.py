...
This module provides a deterministic simulated training loop and bookkeeping
that mirrors the intended experiment orchestration (pretrain, warm-start,
adaptation, extension) without requiring full model internals yet.
"""
...
class Phase1Simulator:
    ...
    def _estimate_model_size_m(self) -> float:
        ...
        per_layer = (4 * mcfg.hidden_size * mcfg.hidden_size) + (3 * mcfg.hidden_size * mcfg.intermediate_size)
    ...
    def run(self) -> None:
        ...
        if start_step >= total_steps:
            self.logger.info("Phase 1 simulator: start_step=%d >= total_steps=%d; nothing to run.", start_step, total_steps)
            return
        ...
        with self.artifacts.metrics_path.open("a", encoding="utf-8") as metrics_file:
            for step in range(start_step, total_steps):
                ...
                grad_norm = max(0.05, 1.5 - 0.9 * progress + (self.rng.random() - 0.5) * 0.06)
                ...
                should_save = (save_freq > 0 and step > 0 and step % save_freq == 0) or step == total_steps - 1
                if should_save:
                    ckpt_payload = {
        ...
        self.logger.info("Phase 1 simulator finished in %.2fs; metrics=%s", elapsed, self.artifacts.metrics_path)
