...
def _bootstrap_remote(ssh_key: Path, ssh_target: str) -> None:
    command = (
        ...
        "  sudo apt-get update -y && sudo apt-get install -y curl rsync git unzip awscli && break; "
        "  if [ \"$attempt\" -eq 3 ]; then exit 100; fi; "
        "  sleep 10; "
        "done; "
...
def _load_availability() -> list[dict]:
    payload = json.loads(_run_capture(["prime", "availability", "list", "--no-group-similar", "--output", "json"]))
...
def _select_target(rows: list[dict]) -> dict | None:
    ...
    for row in rows:
        ...
        if count != 8:
            continue
        if "RTX PRO 6000B 96GB" not in gpu_type:
            continue
        matches.append(row)
...
def _create_pod(row: dict, *, pod_name: str, disk_size_gb: int, image: str, log_path: Path) -> str:
    vcpus = _safe_int(row.get("vcpus"), 240)
    memory = _safe_int(row.get("memory_gb"), 720)
    cmd = [
        "prime",
        "pods",
        "create",
...
def _launch_gate_batch(ssh_key: Path, ssh_target: str) -> int:
    command = (
        "set -euo pipefail; "
        f"cd {shlex.quote(REMOTE_REPO_ROOT)}; "
        "export PATH=\"$HOME/.local/bin:$PATH\"; "
        "cat > /root/runlogs/h100_b_gate_wrapper.sh <<'SH'\n"
        "set -euo pipefail\n"
        f"cd {shlex.quote(REMOTE_REPO_ROOT)}\n"
        "export PATH=\"$HOME/.local/bin:$PATH\"\n"
        ...
        "--stop-after-gates\n"
        "SH\n"
        "chmod +x /root/runlogs/h100_b_gate_wrapper.sh; "
        "rm -f /root/runlogs/h100_b_gate.exit /root/runlogs/h100_b_gate.pid; "
        "nohup bash -lc '/root/runlogs/h100_b_gate_wrapper.sh > /root/runlogs/h100_b_gate.log 2>&1; "
        "rc=$?; printf \"%s\\n\" \"$rc\" > /root/runlogs/h100_b_gate.exit' "
        "</dev/null >/dev/null 2>&1 & "
        "echo $! > /root/runlogs/h100_b_gate.pid; "
        "cat /root/runlogs/h100_b_gate.pid"
    )
...
def _remote_status_snapshot(ssh_key: Path, ssh_target: str, pid: int) -> dict:
    py = (
        ...
        "summary = {}\n"
        f"for run_id in ['adapt-125m-e2e-8K-from-fa-gate2', 'pretrain-125m-e2e-gate2', 'pretrain-125m-fa']:\n"
        f"    ckpt = pathlib.Path('{REMOTE_REPO_ROOT}/checkpoints/{CANONICAL_PAPER_RUN_ID}') / run_id / 'latest.json'\n"
        "    summary[run_id] = load(ckpt)\n"
        f"summary['batch_summary'] = load(pathlib.Path('{REMOTE_REPO_ROOT}/reports/paper/{CANONICAL_PAPER_RUN_ID}/split_batches/h100_b.json'))\n"
        f"exit_file = pathlib.Path('/root/runlogs/h100_b_gate.exit')\n"
...
def _sync_lightweight_results(ssh_key: Path, ssh_target: str, local_out_root: Path) -> None:
    ...
    sync_pairs = [
        ...
        (
            f"{REMOTE_REPO_ROOT}/experiments/{CANONICAL_PAPER_RUN_ID}/S2_ADAPT_125M_GATE",
            local_out_root / "experiments" / "S2_ADAPT_125M_GATE",
        ),
        (
            f"{REMOTE_REPO_ROOT}/experiments/{CANONICAL_PAPER_RUN_ID}/S3_PRETRAIN_E2E_125M_GATE",
            local_out_root / "experiments" / "S3_PRETRAIN_E2E_125M_GATE",
        ),
        (
            "/root/runlogs",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Launch the current 8x RTX PRO 6000B 96GB row, run the 125M Protocol R h100_b "
            "gate-only batch, sync lightweight artifacts locally, and terminate the pod."
        )
    )
    ...
    parser.add_argument("--pod-name-prefix", default="ttt-e2e-125m-h100b-rtx6000")
...
def main() -> int:
    ...
    if missing:
        raise RuntimeError(f"Missing required env values for h100_b watcher: {', '.join(missing)}")
    ssh_key = _prime_ssh_key_path()
    args.log_dir.mkdir(parents=True, exist_ok=True)
    watch_log = args.log_dir / "h100_b_rtx6000_watch.jsonl"
    ...
    if target is None:
        _print("No 8x RTX PRO 6000B 96GB row available right now.")
        return 1

    stamp = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
    pod_name = f"{cfg.pod_name_prefix}-{stamp}"
    local_out_root = args.log_dir / f"h100_b_gate_{stamp}"
    pod_id = None
    try:
        _print(
        ...
        pid = _launch_gate_batch(ssh_key, ssh_target)
        ...
        _print("h100_b RTX6000 gate watch completed.")
        if pod_id:
