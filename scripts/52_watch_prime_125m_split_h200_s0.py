...
HF_STAGE_PREFIX = "ext-125m-fa-32K"
...
def _launch_batch(ssh_key: Path, ssh_target: str) -> int:
    command = (
        ...
        "cat > /root/runlogs/h200_s0_wrapper.sh <<'SH'\n"
        "set -euo pipefail\n"
        f"cd {shlex.quote(REMOTE_REPO_ROOT)}\n"
        "export PATH=\"$HOME/.local/bin:$PATH\"\n"
        ...
        "--skip-existing\n"
        "SH\n"
        "chmod +x /root/runlogs/h200_s0_wrapper.sh; "
        "rm -f /root/runlogs/h200_s0.exit /root/runlogs/h200_s0.pid; "
        "nohup bash -lc '/root/runlogs/h200_s0_wrapper.sh > /root/runlogs/h200_s0.log 2>&1; "
        "rc=$?; printf \"%s\\n\" \"$rc\" > /root/runlogs/h200_s0.exit' "
        "</dev/null >/dev/null 2>&1 & "
        "echo $! > /root/runlogs/h200_s0.pid; "
        "cat /root/runlogs/h200_s0.pid"
    )
...
def _remote_status_snapshot(ssh_key: Path, ssh_target: str, pid: int) -> dict:
    py = (
        ...
        "summary = {}\n"
        f"for run_id in ['ext-125m-fa-32K', 'pretrain-125m-fa']:\n"
        f"    ckpt = pathlib.Path('{REMOTE_REPO_ROOT}/checkpoints/{CANONICAL_PAPER_RUN_ID}') / run_id / 'latest.json'\n"
        "    summary[run_id] = load(ckpt)\n"
        f"summary['batch_summary'] = load(pathlib.Path('{REMOTE_REPO_ROOT}/reports/paper/{CANONICAL_PAPER_RUN_ID}/split_batches/h200_s0.json'))\n"
        f"summary['hf_export_manifest'] = pathlib.Path('{REMOTE_REPO_ROOT}/experiments/{CANONICAL_PAPER_RUN_ID}/S0_125M/ext-125m-fa-32K/hf_export_manifest.json').exists()\n"
        "exit_file = pathlib.Path('/root/runlogs/h200_s0.exit')\n"
...
def _verify_hf_export(repo_id: str, token: str) -> bool:
    ...
    prefix = f"{CANONICAL_PAPER_RUN_ID}/stages/S0_125M/{HF_STAGE_PREFIX}/"
    required = {
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "h200_s0 split batch on the pod, verify the S0 export on HF, sync lightweight "
    ...
    parser.add_argument("--pod-name-prefix", default="ttt-e2e-125m-h200s0")
...
def main() -> int:
    ...
    if missing:
        raise RuntimeError(f"Missing required env values for h200_s0 watcher: {', '.join(missing)}")
    ssh_key = _MOD._prime_ssh_key_path()
    args.log_dir.mkdir(parents=True, exist_ok=True)
    watch_log = args.log_dir / "h200_s0_watch.jsonl"
    ...
    while True:
        ...
        local_out_root = args.log_dir / f"h200_s0_{stamp}"
        pod_id = None
        try:
            _MOD._print(
            ...
            _MOD._append_jsonl(
                watch_log,
                {"event": "hf_verify", "pod_id": pod_id, "verification": {"S0_125M": hf_ok}, "local_out_root": str(local_out_root)},
            )
            if not hf_ok:
                raise RuntimeError("HF verification failed for S0_125M export.")

            _MOD._append_jsonl(
                watch_log,
                {"event": "watch_complete", "pod_id": pod_id, "local_out_root": str(local_out_root), "verification": {"S0_125M": hf_ok}},
            )
            _MOD._print("h200_s0 watch completed successfully.")
            if pod_id:
